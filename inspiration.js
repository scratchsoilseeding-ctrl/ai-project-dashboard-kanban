(() => {
  ...
  const ANALYSIS_STEPS = ["验证云端与分析请求","解析作品链接与公开信息","获取并标准化视频或图集","Qwen 理解画面、声音与字幕","整理并校验报告字段","将内容写入报告编辑器"];
  ...
  let analysisUserCancelled = false;
  let analysisProgress = {status:"idle",stage:"connecting",stepIndex:1,percent:6,message:"准备连接分析服务…",startedAt:0,error:""};
  ...
  function analysisElapsed(){return analysisProgress.startedAt?Math.max(0,Math.floor((Date.now()-analysisProgress.startedAt)/1000)):0;}
  function progressStageName(stepIndex=analysisProgress.stepIndex){return ANALYSIS_STEPS[Math.max(0,Math.min(ANALYSIS_STEPS.length-1,Number(stepIndex||1)-1))];}
  function renderAnalysisProgress(){
    const body=$("#analysisProgressBody");if(!body)return;const failed=analysisProgress.status==="failed",cancelled=analysisProgress.status==="cancelled",complete=analysisProgress.status==="complete",percent=complete?100:Math.max(0,Math.min(99,Number(analysisProgress.percent)||0)),elapsed=analysisElapsed();
    body.innerHTML=`<div class="ap-heading"><div><div class="ap-kicker">ANALYSIS JOB · ${esc(editingId?editingId.slice(-8):"NEW")}</div><h2>${failed?"解析失败":cancelled?"解析已取消":complete?"拆解报告已完成":"正在拆解作品"}</h2><p class="ap-lead">${failed?"报告没有被写入。下面已经标明失败步骤和服务返回的原因。":cancelled?"本次请求已停止，原有报告内容没有被覆盖。":complete?"Qwen 返回的内容已通过字段检查，并写入右侧报告编辑器。":"视频分析通常需要 1–4 分钟；可以收起这个进度窗口，当前页面会继续处理。"}</p></div><button class="ap-close" type="button" data-ap-background aria-label="收起进度">×</button></div><div class="ap-grid"><section class="ap-card"><div class="ap-list">${ANALYSIS_STEPS.map((label,index)=>{const number=index+1,done=complete||number<analysisProgress.stepIndex,active=!failed&&!cancelled&&!complete&&number===analysisProgress.stepIndex,broken=(failed||cancelled)&&number===analysisProgress.stepIndex;return `<div class="ap-step ${done?"done":""} ${active?"active":""} ${broken?"failed":""}"><span class="ap-dot">${done?"✓":broken?"!":number}</span><b>${esc(label)}</b><small>${done?"完成":active?"处理中":broken?failed?"失败":"已取消":"等待"}</small></div>`;}).join("")}</div><div class="ap-message ${failed?"error":""}">${failed?`<span class="ap-error-stage">失败阶段 · ${esc(progressStageName())}</span>`:""}${esc(analysisProgress.message||analysisProgress.error||"正在等待服务响应…")}</div><div class="ap-actions">${failed||cancelled?`<button class="secondary" type="button" data-ap-back>返回修改</button><button class="primary" type="button" data-ap-retry>重新解析</button>`:complete?`<button class="primary" type="button" data-ap-report>查看拆解报告</button>`:`<button class="secondary" type="button" data-ap-background>收起窗口，后台继续</button><button class="secondary" type="button" data-ap-cancel>取消解析</button>`}</div></section><aside class="ap-side ${failed||cancelled?"failed":complete?"complete":""}"><div><div class="ap-kicker">${failed?"FAILED AT STEP "+analysisProgress.stepIndex:cancelled?"CANCELLED":complete?"COMPLETE":"DECODING"}</div><div class="ap-percent">${failed?"—":cancelled?"×":percent+"%"}</div><div class="ap-meter"><i style="width:${percent}%"></i></div><span class="ap-elapsed">已用时 ${elapsed} 秒</span></div><p>${failed?esc(analysisProgress.error||"服务没有返回更多信息。请复制这段错误交给我继续排查。"):cancelled?"你可以修改链接或改用备用上传后重新开始。":complete?"所有报告字段均已通过最基本的非空校验。":"当前步骤和服务消息会持续更新；如果失败，会停在具体步骤并显示原始原因。"}</p></aside></div>`;
  }
  function openAnalysisProgress(){const dialog=$("#analysisProgressDialog");if(!dialog.open)dialog.showModal();renderAnalysisProgress();}
  function beginAnalysisProgress(){analysisUserCancelled=false;analysisProgress={status:"processing",stage:"connecting",stepIndex:1,percent:6,message:"正在建立安全连接并验证云同步配置…",startedAt:Date.now(),error:""};clearInterval(analysisClock);analysisClock=setInterval(()=>{if($("#analysisProgressDialog")?.open)renderAnalysisProgress();},1000);openAnalysisProgress();}
  function updateAnalysisProgress(progress={}){analysisProgress={...analysisProgress,status:"processing",stage:progress.stage||analysisProgress.stage,stepIndex:Number(progress.stepIndex)||analysisProgress.stepIndex,percent:Number(progress.percent)||analysisProgress.percent,message:progress.message||analysisProgress.message};renderAnalysisProgress();}
  function failAnalysisProgress(error){clearInterval(analysisClock);const cancelled=analysisUserCancelled||error?.name==="AbortError";analysisProgress={...analysisProgress,status:cancelled?"cancelled":"failed",message:cancelled?"已按你的要求取消解析。":String(error?.message||"解析失败，但服务没有返回具体原因。"),error:String(error?.message||"")};openAnalysisProgress();}
  function finishAnalysisProgress(){clearInterval(analysisClock);analysisProgress={...analysisProgress,status:"complete",stage:"complete",stepIndex:ANALYSIS_STEPS.length,percent:100,message:"拆解报告已生成并写入编辑器。"};renderAnalysisProgress();}
  ...
  function applyAnalysis(result,model="",source=""){const value=result?.result||result?.analysis||result?.report||result||{};$("#itemHighlights").value=String(value.highlights||value.contentHighlights||value.overview||value["内容亮点"]||"");$("#itemTimeline").value=timelineText(value.timeline||value.segments||value["逐段时间轴"]);$("#itemStructure").value=String(value.structure||value.contentStructure||value["内容结构"]||"");$("#itemAudiovisual").value=String(value.audiovisual||value.audioVisual||value["画面与声音配合"]||"");$("#itemReproducibility").value=String(value.reproducibility||value.replicability||value["可复制性分析"]||"");$("#itemSteps").value=safeArray(value.steps||value.productionSteps||value["制作步骤"]).join("\n");$("#itemEvidence").value=safeArray(value.evidence||value.observations||value["证据"]).join("\n");$("#itemScore").value=String(Math.max(1,Math.min(5,Number(value.copyScore||value.replicationScore)||3)));$("#itemTags").value=safeArray(value.tags||value.keywords||value["标签"]).join(", ");pendingAnalysisModel=model;pendingAnalysisSource=source;pendingAnalyzedAt=new Date().toISOString();}
  function applyResolvedMetadata(resolved={}){if(resolved.title&&!$("#itemTitle").value.trim())$("#itemTitle").value=resolved.title;if(resolved.creator&&!$("#itemCreator").value.trim())$("#itemCreator").value=resolved.creator;if(resolved.platform)$("#itemPlatform").value=resolved.platform;if(resolved.mediaType)$("#itemType").value=resolved.mediaType==="video"?"视频":"图集";if(resolved.coverUrl&&!pendingCover){pendingCover={name:"作品封面",dataUrl:resolved.coverUrl};renderCover();}}
  async function analyzeMediaForm(signal){
    ...
    const metadata={sourceUrl:source,title:$("#itemTitle").value.trim(),creator:$("#itemCreator").value.trim(),platform:$("#itemPlatform").value.trim(),type:$("#itemType").value.trim(),userObservation:$("#itemNote").value.trim()};const response=await callVideoStream({path,videoUrl:directVideo,sourceUrl:!path&&!directVideo?source:"",metadata},progress=>{if(progress.resolved)applyResolvedMetadata(progress.resolved);updateAnalysisProgress(progress);if(progress.message){$("#analysisHint").textContent=progress.message;$("#analyzeBtn").textContent=progress.stage==="model"?"Qwen 分析中…":progress.stage==="report"?"正在整理报告…":"正在读取作品…";}},signal),resolved=response.resolved||{};
  ...
  async function analyzeForm(){
    ...
    analysisController?.abort();analysisController=new AbortController();beginAnalysisProgress();
    try{if(hasMedia)await analyzeMediaForm(analysisController.signal);else{updateAnalysisProgress({stage:"model",stepIndex:4,percent:62,message:"正在根据标题和你的观察生成文字辅助分析…"});await analyzeTextForm();}finishAnalysisProgress();toast(hasMedia?"作品深度拆解完成":"文本分析完成");}catch(error){if(error?.analysisStep){analysisProgress.stepIndex=error.analysisStep;analysisProgress.stage=error.analysisStage||analysisProgress.stage;}$("#analysisHint").textContent=error.message;failAnalysisProgress(error);toast(`解析失败：${error.message}`);}finally{button.disabled=false;analysisController=null;updateAnalysisMode();}
  }
  ...
  $("#analysisProgressBody").addEventListener("click",event=>{
    const button=event.target.closest("button");if(!button)return;
    if(button.matches("[data-ap-background],[data-ap-back]"))return $("#analysisProgressDialog").close();
    if(button.matches("[data-ap-cancel]")){analysisUserCancelled=true;analysisController?.abort();return;}
    if(button.matches("[data-ap-retry]")){$("#analysisProgressDialog").close();setTimeout(analyzeForm,0);return;}
    if(button.matches("[data-ap-report]")){$("#analysisProgressDialog").close();$(".analysis-editor").scrollIntoView({behavior:"smooth",block:"start"});setTimeout(()=>$("#itemHighlights").focus(),350);}
  });
  $("#analysisProgressDialog").addEventListener("cancel",event=>{event.preventDefault();$("#analysisProgressDialog").close();});
