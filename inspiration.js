(() => {
  ...
  async function callVideoStream(payload,onProgress=()=>{}){
    if(!hasCloud())throw new Error("请先在项目看板中配置云同步");
    const response=await fetch(videoServiceUrl(),{method:"POST",headers:{apikey:cloudConfig.anonKey,"Content-Type":"application/json",Accept:"text/event-stream"},body:JSON.stringify({action:"analyze-stream",accessKey:cloudConfig.spaceId,...payload})});
    if(!response.ok){const raw=await response.text();let detail;try{detail=JSON.parse(raw);}catch(error){detail={error:raw};}throw new Error(detail.error||`作品分析请求失败（${response.status}）`);}
