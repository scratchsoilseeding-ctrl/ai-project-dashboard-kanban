(() => {
  ...
  function renderVideo(){
    ...
    if(pendingVideoFile){status.textContent=pendingVideoFile.name;meta.textContent=`待上传 · ${formatBytes(pendingVideoFile.size)} · 保存或分析时自动上传`;remove.hidden=false;return;}
    if(pendingVideo?.path){status.textContent=pendingVideo.name||"已上传视频";meta.textContent=`Supabase 私密存储${pendingVideo.size?` · ${formatBytes(pendingVideo.size)}`:""}`;remove.hidden=false;return;}
    status.textContent="选择视频文件";meta.textContent="上传后由 Qwen-Omni 同时理解画面、讲话和环境声音";remove.hidden=true;
  }
  function setOmniState(text,state=""){$("#omniState").textContent=text;$("#omniState").className=state;}
  function openEditor(id=""){
    const item=inspirations().map(normalizeItem).find(entry=>entry.id===id);editingId=item?.id||"";pendingCover=item?.cover?clone(item.cover):null;pendingVideo=item?.video?clone(item.video):null;pendingVideoFile=null;removedVideoPath="";pendingAnalysisModel=item?.analysis.model||"";pendingAnalyzedAt=item?.analysis.analyzedAt||"";
    ...
    renderCover();renderVideo();$("#analysisHint").textContent="上传视频后，Qwen-Omni 会同时读取画面与声音；只有作品链接时仍会使用原来的文本分析。";setOmniState(hasCloud()?"正在检测 Qwen-Omni 服务…":"请先在项目看板配置云同步");$("#editorDialog").showModal();$("#itemUrl").focus();if(hasCloud())checkOmniService();
  }
  function formItem(){
  ...
  function applyAnalysis(result,model=""){$("#itemHighlights").value=String(result.highlights||result.overview||"");$("#itemTimeline").value=timelineText(result.timeline);$("#itemStructure").value=String(result.structure||"");$("#itemAudiovisual").value=String(result.audiovisual||"");$("#itemReproducibility").value=String(result.reproducibility||"");$("#itemSteps").value=safeArray(result.steps).join("\n");$("#itemEvidence").value=safeArray(result.evidence).join("\n");$("#itemScore").value=String(Math.max(1,Math.min(5,Number(result.copyScore)||3)));$("#itemTags").value=safeArray(result.tags).join(", ");pendingAnalysisModel=model;pendingAnalyzedAt=new Date().toISOString();}
  async function analyzeVideoForm(){
    if(pendingVideoFile)await uploadSelectedVideo();const directVideo=safeUrl($("#itemVideoUrl").value.trim()),path=pendingVideo?.path;if(!path&&!directVideo)throw new Error("请先上传视频文件，或填写可直接访问的视频地址");
    const metadata={sourceUrl:$("#itemUrl").value.trim(),title:$("#itemTitle").value.trim(),creator:$("#itemCreator").value.trim(),platform:$("#itemPlatform").value.trim(),type:$("#itemType").value.trim(),userObservation:$("#itemNote").value.trim()};const response=await callVideoService("analyze",{path,videoUrl:directVideo,metadata});applyAnalysis(response.result,response.model);$("#analysisHint").textContent=`${response.model||"Qwen-Omni"} 已完成视频深度拆解。请检查并保存卡片。`;setOmniState(`${response.model||"Qwen-Omni"} 已就绪`,"ready");
  }
  async function analyzeTextForm(){
    if(!getAIConfig()){openAISettings("没有视频素材。请配置文本模型，或返回编辑器上传视频。");return false;}const url=$("#itemUrl").value.trim(),title=$("#itemTitle").value.trim()||`${$("#itemPlatform").value.trim()||platformFromUrl(url)||hostLabel(url)} 灵感`,note=$("#itemNote").value.trim();if(!url)throw new Error("请先粘贴作品链接");const system="你是创意作品拆解教练。不能访问链接时必须基于用户提供的信息分析，不要假装看过页面。只返回 JSON。";const prompt=`参考作品：${JSON.stringify({url,title,creator:$("#itemCreator").value.trim(),platform:$("#itemPlatform").value.trim(),type:$("#itemType").value.trim(),userObservation:note})}\n只返回：{"highlights":"80-160字","reproducibility":"80-160字","copyScore":1到5,"steps":["4到8个具体步骤"],"tags":["2到5个短标签"]}`;const config=getAIConfig(),result=parseAIJson(await callAI(system,prompt));applyAnalysis(result,config.model);$("#analysisHint").textContent="这是基于标题与观察生成的文本分析，并未读取视频内容。";return true;
  }
  async function analyzeForm(){
    const hasVideo=Boolean(pendingVideoFile||pendingVideo?.path||safeUrl($("#itemVideoUrl").value.trim())),button=$("#analyzeBtn"),oldText=button.textContent;button.disabled=true;button.textContent=hasVideo?"Qwen 正在看完整视频…":"AI 正在分析文字…";$("#analysisHint").textContent=hasVideo?"正在同时理解口播、字幕、镜头、音乐和内容结构，较长视频需要几分钟…":"正在根据标题和你的观察进行辅助分析…";
    try{if(hasVideo)await analyzeVideoForm();else await analyzeTextForm();toast(hasVideo?"视频深度拆解完成":"文本分析完成");}catch(error){$("#analysisHint").textContent=error.message;toast(error.message);}finally{button.disabled=false;button.textContent=oldText;}
  }
